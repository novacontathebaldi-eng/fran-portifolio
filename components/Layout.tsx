...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  // Refactored Nav Logic for Smooth Transitions - GLASSMORPHISM
  // 1. Scrolled: Compact, white glass with subtle shadow
  // 2. Top + Transparent Page: Tall, dark glass overlay
  // 3. Top + Standard Page: Tall, white background

  const navClasses = isScrolled
    ? 'bg-white/90 backdrop-blur-xl shadow-sm py-4 border-b border-gray-100'
    : isTransparentNavPage
      ? 'header-glass py-6'
      : 'bg-white/95 backdrop-blur-xl py-6 border-b border-gray-100';
  ...
  const logoClasses = isMenuOpen
    ? 'text-[#1a1a1a]'
    : textColorCondition ? 'text-[#1a1a1a]' : 'text-white';

  const linkClasses = textColorCondition
    ? 'text-[#1a1a1a]/80 hover:text-[#c9a962] transition-colors duration-300'
    : 'text-white/80 hover:text-[#c9a962] transition-colors duration-300';
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-primary bg-white">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-700 ease-in-out ${isMenuOpen ? 'bg-transparent' : navClasses}`}>
        <div className="container mx-auto px-6 flex justify-between items-center relative">
          ...
          <Link to="/" onClick={handleLinkClick} className={`text-xl md:text-2xl font-serif tracking-tight font-bold z-[60] relative uppercase transition-colors duration-300 pointer-events-auto ${logoClasses}`}>
            FRAN<span className="text-[#c9a962]">.</span>
          </Link>
      ...
      {
        isMenuOpen && (
          <div className="fixed inset-0 bg-white/95 backdrop-blur-xl z-[45] flex flex-col pt-24 pb-8 px-6 animate-fadeIn text-[#1a1a1a] md:hidden overflow-y-auto pointer-events-auto">
            <div className="flex flex-col space-y-6 flex-grow">
              <Link to="/" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Início</Link>
              <Link to="/about" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Sobre</Link>
              <Link to="/office" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">O Escritório</Link>
              <Link to="/portfolio" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Portfólio</Link>
              <Link to="/cultural" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Cultura</Link>
              {settings.enableShop && (
                <Link to="/shop" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Loja</Link>
              )}
              <Link to="/services" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Serviços</Link>
              <Link to="/contact" onClick={handleLinkClick} className="text-3xl font-serif font-light hover:text-[#c9a962] transition border-b border-white/10 pb-4">Contato</Link>

              <div className="pt-4 space-y-4">
                <Link to={currentUser ? "/profile" : "/auth"} onClick={handleLinkClick} className="flex items-center space-x-3 text-lg font-medium hover:text-[#c9a962] transition">
                  <User className="w-5 h-5" />
                  <span>Minha Conta</span>
                </Link>
                <button onClick={() => { setIsMenuOpen(false); setSearchOpen(true); }} className="flex items-center space-x-3 text-lg font-medium hover:text-[#c9a962] transition w-full text-left">
                  <Search className="w-5 h-5" />
                  <span>Buscar</span>
                </button>
                {settings.enableShop && (
                  <Link to="/cart" onClick={handleLinkClick} className="flex items-center space-x-3 text-lg font-medium hover:text-[#c9a962] transition relative">
                    <ShoppingBag className="w-5 h-5" />
                    <span>Carrinho</span>
                    {cartCount > 0 && (
                      <span className="ml-2 bg-[#c9a962] text-black text-xs font-bold px-2 py-0.5 rounded-full">
                        {cartCount}
                      </span>
                    )}
                  </Link>
                ...
                {currentUser?.role === 'admin' && (
                  <Link to="/admin" onClick={handleLinkClick} className="text-lg font-bold text-[#c9a962] pt-2 block">Acessar Admin</Link>
                )}
              </div>
            </div>
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div className="md:col-start-3">
              <h4 className="text-xs font-bold uppercase tracking-widest mb-8 text-[#c9a962]">Navegação</h4>
            ...
            <div>
              <h4 className="text-xs font-bold uppercase tracking-widest mb-8 text-[#c9a962]">Instalar App</h4>
