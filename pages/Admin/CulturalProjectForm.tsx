...
export const CulturalProjectForm: React.FC = () => {
  ...
  const { culturalProjects, addCulturalProject, updateCulturalProject, showToast } = useProjects();

  const isEditing = Boolean(id);
  const existingProject = culturalProjects.find(p => p.id === id);
  ...
  const [formData, setFormData] = useState<Partial<CulturalProject>>({
    title: '',
    category: 'Patrimônio',
    ...
    <div className="min-h-screen bg-gray-50 font-sans p-4 md:p-8 text-gray-900">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-3xl font-serif font-bold text-black">{isEditing ? 'Editar Projeto Cultural' : 'Novo Projeto Cultural'}</h1>
        </div>
            ...
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input name="title" value={formData.title} onChange={handleChange} className="w-full border border-gray-300 p-3 rounded text-sm bg-white text-black focus:outline-none focus:border-black transition" placeholder="Ex: Restauro do Museu" />
                </div>
                <div>
                  <label className="block text-xs font-bold uppercase text-gray-500 mb-2">Categoria</label>
                  <select name="category" value={formData.category} onChange={handleChange} className="w-full border border-gray-300 p-3 rounded text-sm bg-white text-black focus:outline-none focus:border-black transition">
                ...
                <div>
                  <label className="block text-xs font-bold uppercase text-gray-500 mb-2">Parceiros Institucionais</label>
                  <input name="partners" value={formData.partners || ''} onChange={handleChange} className="w-full border border-gray-300 p-3 rounded text-sm bg-white text-black focus:outline-none focus:border-black transition" placeholder="Ex: IPHAN, Prefeitura..." />
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white p-6 md:p-8 rounded-xl shadow-sm border border-gray-200 min-h-[600px]">
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pb-6 border-b border-gray-100 gap-4">
                <div>
                  <h2 className="text-2xl font-bold font-serif text-black">Narrativa Histórica</h2>
