...
export const Cultural: React.FC = () => {
  ...
  const sortedProjects = [...culturalProjects].sort((a, b) => {
      ...
      <div className="container mx-auto px-6 mt-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  <div>
                    ...
                    <div className="space-y-2 text-sm text-secondary">
                      <label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" className="accent-black rounded" /> <span>Patrimônio</span></label>
                      <label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" className="accent-black rounded" /> <span>Exposição</span></label>
                      <label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" className="accent-black rounded" /> <span>Restauro</span></label>
                      <label className="flex items-center space-x-2 cursor-pointer"><input type="checkbox" className="accent-black rounded" /> <span>Pesquisa</span></label>
                    </div>
                  </div>
                  <div>
        ...
        {culturalProjects.length === 0 ? (
           <div className="py-20 text-center text-gray-400">
             <p className="text-xl font-serif">Nenhum projeto cultural encontrado.</p>
           </div>
        ) : view === 'grid' ? (
          <motion.div 
            layout
          ...
          >
            {sortedProjects.map((project) => (
              <Link to={`/cultural/${project.id}`} key={project.id} className="group block h-full flex flex-col">
